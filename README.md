### Hi there 👋

  <a href="https://www.linkedin.com/in/apolo-ferreira-santos-1860a6b6/" target="_blank"><img src="https://img.shields.io/badge/-LinkedIn-%230077B5?style=for-the-badge&logo=linkedin&logoColor=white" target="_blank"></a> 

## 🚀 About Me
I've always been passionate about everything that 
involves process automation. 
As a child I took my toys apart to see how they worked. 
My leisure hours are filled with technology channels and pages.

I'll share a little bit of what was important in my 
learning trajectory, mainly in the areas of Data Science and Machine Learning.

  # A list of learning resources for Data Science

This file contains a list with some of the contents that contributed to my 
learning in programming, Automation, Data Science, Machine Learning, 
Blockchain and technology in general.

Soon, I will also be adding materials of interest to me, 
such as future courses and books that I would like to do or will do, 
as well as projects that I am developing at the moment.

## 🛠 Skills
- Python
- C#
- C++
- JavaScript
- SQL
- Go
- Solidity
- html
...

  
## Favorites Blogs e WebSites

* [Medium](https://medium.com/) :green_heart: 
* [Analytics Vidhya](https://www.analyticsvidhya.com/blog/)
* [KDnuggets](https://www.kdnuggets.com/)
* [Machine Learning Mastery](https://machinelearningmastery.com/)
* [Blog DSA](https://blog.dsacademy.com.br/)
* [TED](https://www.ted.com/)

## Online courses :computer:
### Data Science Academy

#### [Formação Engenheiro de Machine Learning](https://www.datascienceacademy.com.br/bundle/formacao-engenheiro-de-machine-learning)
- [x] Fundamentos de Engenharia de Software
- [x] Machine Learning em Python e C++
- [x] Deploy de Modelos de Machine Learning
- [x] Machine Learning Automation
- [x] Módulos Extras de Capacitação Profissional

#### [Formação Cientista de Dados](https://www.datascienceacademy.com.br/bundle/formacao-cientista-de-dados)
- [ ] Big Data Analytics com R e Microsoft Azure Machine Learning
- [x] Big Data Real-Time Analytics com Python e Spark
- [ ] Engenharia de Dados com Hadoop e Spark
- [x] Machine Learning
- [ ] Business Analytics
- [ ] Visualização de Dados e Design de Dashboards
- [ ] Preparação Para Carreia de Cientista de Dados

#### [Formação Engenheiro Blockchain](https://www.datascienceacademy.com.br/bundle/formacao-engenheiro-blockchain)
- [x] Blockchain Fundamentos
- [x] Programação Para Blockchain
- [ ] Desenvolvimento de Aplicações Descentralizadas
- [ ] Inteligência Artificial Aplicada a Finanças
- [x] Deep Learning I
- [ ] Deep Learning II

### Udemy
- [x] Interactive Python Dashboards with Ploty and Dash
- [x] Feature Engineering for Machine Learning
- [x] Hyperparameter Optimization for Machine Learning
- [x] Deployment of Machine Learning Models
- [ ] Testing and Monitoring Machine Learning Model Deployments
- [x] Feature Selection for Machine Learning
- [x] Machine Learning with Imbalanced Data
- [x] Programação Web com Python e Django Framework: Essencial
- [x] Git e Github do básico ao avançado (c/ gits e GitHub Pages)
- [x] Curso Web Moderno Completo com JavaScript 2021 + Projetos
- [x] JavaScript - Curso Completo com 6 Projetos Reais
- [x] Dominando Banco de Dados com MySQL
- [x] Desenvolvimento de Sistemas com C#
- [x] Inteligência Artificial: Algoritmos Genéticos - TSP
- [x] Machine Learning e Data Science com Python
- [x] Redes Neurasi Artificiais em Python
- [x] Contratos Inteligentes: Programação Solidty para Etherium
- [x] Formação Engenheiro de Blockchain: O Curso Completo!
- [x] Blockchain: o que todos devem saber
- [ ] Deploy Machine Learning Models on GCP + AWS Lambda (Docker)
- [ ] 20+ End-To-End Machine Learning Projects & Deployment 2021
- [ ] Curso de HTML 5 Completo e com Projetos Práticos para WEB
- [ ] FastAPI Full Stack Web Development (API + Webapp)

## Tools for Deploy
- Heroku
- Streamlit
- Ploty and Dash Dashboards
- Flask
- FastAPI
- Html and Css

## Cloud Computer AWS
- EC2
- S3
- RDS
- SageMaker
- Route 53
- Elastic Balancing
- CloundFront
- Glue
- Lambda

## Work Environment
- Anaconda
- Docker
- Git
- Linux

## Main Python Libraries
- [x] Numpy
- [x] Pandas
- [x] Sckit-Learn
- [x] Matplotlib
- [x] Seaborn
- [x] Keras

## Books :green_book:
O'Reilly
- [x] Hands-On Machine Learning with Scikit-Learn, Keras and TensorFlow (2nd Edition)
- [ ] Data Science on AWS - Implementing End-to-End, Continuos AI and Machine Learning Pipelines

## Data Science Techniques
- Feature Selection
- Feature Engineering
- Imbalanced Data
- Hyperparameter Optimization
- Machine Learning Models

## Some of my Favorite Youtubers
- Siraj Raval
- Ken Jee
- Data School
- The Coding Train
- freeCodeCamp.org
- NeuralNine
- Krish Naik
- Python Engineer
- Python Simplified
- MarinatedPython
- Charming Data
- Stats Wire
- Normalized Nerd
- Coding Tech
- PyData
- Rachit Toshniwal
- DataMites
- Data Professor
- Python Programmer
- When Maths Meet Coding
- Corey Schafer
- Kimberly Fessel
- Computer Science
- Nicholas Renotte
- Jabrils
- Programming with Mosh
- Code Bullet
- sentdex
- Andreas Spiess

## Academic Training

* Control and Automation Engineering

### Academic Experiences
- Development of several applications with programming of electronic boards, mainly ESP32 and Raspberry, with integration to wi-fi, mqtt, bluetooth and tcp/ip protocol.
- Robotics projects with Matlab and automation with PLC
- Windows Desktop Software Development with C# for real use in a mechanical workshop
- Easy detection and recognition software
- Mesh control through P.I.D

### Graduation Final Project
* [You can watch here](https://youtu.be/veMcoGOxjFM)
Soon I will post more details

### Face-to-face Training
- Internet of Things with ESP32 and FreeRTOS
- Simatic Step 7 - basic to advanced
- industrial instrumentation
